        ...
        switch (item.getItemId()) {
            ...
            case R.id.save_pawn:
                ...
                if(text.getText().toString().equals("")){
                    Drawable error = (Drawable)getResources().getDrawable(R.drawable.ic_error_outline_white_24dp);
                    error.setBounds(0, 0, error.getIntrinsicWidth(), error.getIntrinsicHeight());
                    text.setError("Please insert name", error);
                    //text.setHint("Insert Name");
                    //text.setHintTextColor(Color.RED);
                }else {
                    returnIntent.putExtra("name", text.getText().toString());
                    if (colorName == null) {
                        returnIntent.putExtra("color", fromColorPicker("Lavander"));
                    } else {
