...
static final class cleanQueue extends WeakReference
{
    ...
    private static int cleanQueue(ReferenceQueue referencequeue)
    {
        ...
        do
        {
            cleanQueue cleanqueue = (cleanQueue)referencequeue.poll();
            if (cleanqueue == null)
            {
                break;
            }
            RuntimeException runtimeexception = (RuntimeException)cleanqueue.allocationSite.get();
            cleanqueue.super.clear();
            cleanqueue.refs.remove(cleanqueue);
            cleanqueue.allocationSite.clear();
            if (!cleanqueue.shutdown || !cleanqueue.channel.isTerminated())
            {
                Object obj;
                if (cleanqueue.shutdownNow)
                {
                    obj = Level.FINE;
                } else
                {
    ...
    (ManagedChannelOrphanWrapper managedchannelorphanwrapper, ManagedChannel managedchannel, ReferenceQueue referencequeue, ConcurrentMap concurrentmap)
    {
